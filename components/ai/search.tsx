...
export function AISearchInput(props: ComponentProps<'form'>) {
  ...
  const [input, setInput] = useState(() => (typeof localStorage !== 'undefined' ? localStorage.getItem(StorageKeyInput) ?? '' : ''));
...
/** Trigger for TOC footer – same open action, styled as link */
export function AISearchTOCTrigger(props: ComponentProps<'button'>) {
  const { setOpen } = useAISearchContext();
  return (
    <button
      type="button"
      ...
      className={cn(
        'mt-3 flex w-full items-center gap-1.5 ps-3 py-1.5 text-left text-sm text-fd-muted-foreground transition-colors hover:text-fd-foreground',
        props.className,
      )}
      aria-label="Ask AI"
      {...props}
